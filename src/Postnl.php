    ...
    public function call($clientName, $method, $parameter)
    {
        try {
            ...
            if ($exception->faultcode == 's:CIF Framework Message Interceptor') {
                ...
                $soapExceptionData = $exception->detail->CifException->Errors->ExceptionData;
                if (!is_array($soapExceptionData)) {
                    // Cope with inconsistencies in response. Should now always be stdClass[].
                    $soapExceptionData = [$soapExceptionData];
                }

                // Assemble exception data from the response.
                $exceptionData = [];
                foreach ($soapExceptionData as $error) {
